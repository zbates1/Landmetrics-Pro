...
def user_data():
    try:
        ...
        if selected_device_id:
            # Ensure the device belongs to the current user
            device = Device.query.filter_by(id=selected_device_id, user_id=current_user.id).first()
            if not device:
                logger.warning(f"Device ID {selected_device_id} not found for user {current_user.id}")
                flash("Selected device not found.", category='error')
                return redirect(url_for('data_view.user_data'))

            # Retrieve data points for the device
            data_points = DeviceData.query.filter_by(device_id=selected_device_id).order_by(DeviceData.timestamp).all()
            ...
            device_data[str(selected_device_id)] = data_dict
